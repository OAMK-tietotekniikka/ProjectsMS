...
const SelectionDropdown = ({ data, options, toggle, setSelectedData }) => {
    ...
    const handleStudentSelection = () => {
        const filteredData = data.filter((data) => (data.name).toLowerCase() === student.toLowerCase());
        console.log(filteredData);
        console.log(data);
        setSelectedData(filteredData);
    };
    return (_jsx(Container, { style: { paddingLeft: "0px", marginTop: "2%" }, children: _jsx(Col, { children: _jsxs("div", { className: 'dropdown-row', children: [_jsxs(Dropdown, { children: [_jsx(Dropdown.Toggle, { id: "dropdown-basic", className: 'dropdown-toggle', children: t(toggle) }), _jsx(Dropdown.Menu, { children: options.map((item, index) => (_jsx(Dropdown.Item, { onClick: (e) => handleSelection(e, item), className: "dropdown-item", href: "#/action-${index}", children: t(item) }, index))) })] }), _jsxs("div", { style: { marginLeft: "12%" }, children: [showSecondSelection === "studyYear" && (_jsxs(Dropdown, { children: [_jsx(Dropdown.Toggle, { id: "dropdown-basic", className: 'dropdown-toggle', children: "select study year" }), _jsx(Dropdown.Menu, { children: (studyYearList === null || studyYearList === void 0 ? void 0 : studyYearList.map((item, index) => (_jsx(Dropdown.Item, { onClick: (e) => handleStudyYear(e, item), className: "dropdown-item", href: "#/action-${index}", children: item }, index)))) || _jsx(Dropdown.Item, { children: t('noComp') }) })] })), showSecondSelection === "classCode" && (_jsxs(Dropdown, { children: [_jsx(Dropdown.Toggle, { id: "dropdown-basic", className: 'dropdown-toggle', children: "select class" }), _jsx(Dropdown.Menu, { children: (classCodeList === null || classCodeList === void 0 ? void 0 : classCodeList.map((item, index) => (_jsx(Dropdown.Item, { onClick: (e) => handleClassCode(e, item), className: "dropdown-item", href: "#/action-${index}", children: item }, index)))) || _jsx(Dropdown.Item, { children: t('noComp') }) })] })), showSecondSelection === "studentName" && (_jsxs("div", { style: { display: "flex", flexDirection: "column" }, children: [_jsx(Form, { children: _jsx(Form.Control, { type: "text", placeholder: t('enterStudName'), style: { width: "300px", fontSize: "13px" }, onChange: (e) => handleChange(e.target.value), value: student }) }), _jsx(Button, { style: { marginLeft: "20%", marginTop: "4%" }, type: 'button', className: 'addCompany-button', onClick: handleStudentSelection, children: t('selectStudent') })] }))] })] }) }) }));
};
